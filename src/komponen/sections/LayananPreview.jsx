...
const LayananPreview = () => {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 right-0 w-1/3 h-full bg-linear-to-l from-blue-500/5 to-transparent pointer-events-none hidden md:block" />
          ...
          <div className="w-full lg:w-1/2 order-1 lg:order-2">
            <header className="mb-8 md:mb-12">
              ...
              >
                ...
                <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em] md:tracking-[0.6em] text-blue-500">
                  Core Expertise
                </span>
              </FadeIn>

              <FadeIn delay={0.2} direction="up">
                <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-outfit font-bold tracking-tighter text-(--text-primary) uppercase leading-[0.95] mb-6 md:mb-8">
                ...
                <p className="text-(--text-secondary) text-sm md:text-lg lg:text-xl leading-relaxed font-inter mb-8 md:mb-10 max-w-xl">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-6 mb-8 md:mb-12">
                {highlights.map((item, i) => (
                  <FadeIn
                    ...
                    delay={0.4 + i * 0.1}
                    direction="up"
                    className="flex items-center gap-3 md:gap-4 p-3 md:p-4 glass-2 rounded-xl border-l-2 border-l-transparent hover:border-l-blue-500 transition-all duration-500"
                  >
                    <div className={`p-2 bg-white/5 rounded-lg ${item.color}`}>
                      <item.icon size={20} />
                    </div>
                    <span className="text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-(--text-primary)">
                      {item.text}
                    </span>
                  </FadeIn>
                ))}
              ...
              >
                <Link
                  to="/layanan"
                  className="w-full sm:w-auto group inline-flex items-center justify-center gap-3 md:gap-4 px-6 md:px-10 py-4 md:py-5 bg-white text-black font-black text-xs md:text-[10px] uppercase tracking-[0.3em] md:tracking-[0.4em] rounded-sm hover:bg-blue-600 hover:text-white transition-all duration-500"
                >
                  View Services
                  <ArrowRight
                    size={14}
